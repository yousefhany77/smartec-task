import downloadIcon from "../assets/download-image.svg";

interface InputDownloadProps extends React.HTMLAttributes<HTMLButtonElement> {
  onClick: () => void;
}

function DownloadButton(Props: InputDownloadProps) {
  const { onClick, className } = Props;
  return (
    <button
      {...Props}
      className={`bg-white text-purple-500 font-bold px-4 py-2  h-16 w-full rounded-lg transition-colors ease-out duration-200 hover:bg-purple-600 hover:text-white first-letter:
  focus:outline-none focus:ring-2 focus:ring-purple-600 focus:ring-opacity-50
  flex items-center justify-center gap-2 ${className}`}
      onClick={onClick}
    >
      Download
      <svg
        xmlns="http://www.w3.org/2000/svg"
        viewBox="0 0 24 24"
        className="w-5 h-5"
      >
        <path
          fill="currentColor"
          d="M22.71,6.29a1,1,0,0,0-1.42,0L20,7.59V2a1,1,0,0,0-2,0V7.59l-1.29-1.3a1,1,0,0,0-1.42,1.42l3,3a1,1,0,0,0,.33.21.94.94,0,0,0,.76,0,1,1,0,0,0,.33-.21l3-3A1,1,0,0,0,22.71,6.29ZM19,13a1,1,0,0,0-1,1v.38L16.52,12.9a2.79,2.79,0,0,0-3.93,0l-.7.7L9.41,11.12a2.85,2.85,0,0,0-3.93,0L4,12.6V7A1,1,0,0,1,5,6h8a1,1,0,0,0,0-2H5A3,3,0,0,0,2,7V19a3,3,0,0,0,3,3H17a3,3,0,0,0,3-3V14A1,1,0,0,0,19,13ZM5,20a1,1,0,0,1-1-1V15.43l2.9-2.9a.79.79,0,0,1,1.09,0l3.17,3.17,0,0L15.46,20Zm13-1a.89.89,0,0,1-.18.53L13.31,15l.7-.7a.77.77,0,0,1,1.1,0L18,17.21Z"
        />
      </svg>
    </button>
  );
}

export default DownloadButton;
